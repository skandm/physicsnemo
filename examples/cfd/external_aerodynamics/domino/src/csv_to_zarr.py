...
def inspect_csv(input_dir: Path):
    ...
    first = case_dirs[0]
    p_csv = first / PRESSURE_CSV_FILENAME
    v_csv = first / VELOCITY_CSV_FILENAME

    header = 0 if CSV_HAS_HEADER else None

    print(f"\n── CSV inspection (case: {first.name}) ──")
...
def read_csv_surface(
    ...
    p_arr = np.loadtxt(str(pressure_csv), delimiter=",", skiprows=1 if has_header else 0)
    v_arr = np.loadtxt(str(velocity_csv), delimiter=",", skiprows=1 if has_header else 0)

    # Extract coordinates from pressure CSV; assert they match velocity CSV
    ...
    max_coord_diff = np.abs(p_coords - v_coords).max()
    if max_coord_diff > 1e-6:
        raise ValueError(
            f"Coordinate mismatch between pressure.csv and velocity.csv "
            f"(max diff = {max_coord_diff:.2e}). Check that both files share the same grid."
        )

    coords = p_coords * unit_scale  # [N, 3]

    # Extract field columns and stack: [p, vx, vy, vz] → [N, 4]
    pressure = p_arr[:, pressure_cols].astype(np.float32)   # [N, n_p]
    velocity = v_arr[:, velocity_cols].astype(np.float32)   # [N, n_v]
...
def convert_case(
    ...
    p_csv    = case_dir / PRESSURE_CSV_FILENAME
    v_csv    = case_dir / VELOCITY_CSV_FILENAME

    for path, label in [(stl_path, STL_FILENAME), (p_csv, PRESSURE_CSV_FILENAME), (v_csv, VELOCITY_CSV_FILENAME)]:
        if not path.exists():
    ...
    print(
        ...
        f"surface={n_surf} pts / {n_ch} channels"
    )
    return True
